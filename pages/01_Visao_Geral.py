...
if not no_data_message(df):
    ...
    c1.metric("Peças aprovadas", f"{total:,.0f}".replace(",", "."))
    ...
    with right:
        ...
        by_collection = df.groupby("Colecao", dropna=False)[PRODUCTION_COL].sum().sort_values(ascending=False).head(15)
        st.bar_chart(by_collection)

    st.subheader("Resumo por grupo")
    summary = (df.groupby("Grupo", dropna=False)
