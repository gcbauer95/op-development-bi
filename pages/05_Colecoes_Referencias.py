...
if not no_data_message(df):
    ...
    with col1:
        ...
        production_collection = df.groupby("Colecao")[PRODUCTION_COL].sum().sort_values(ascending=False)
        st.bar_chart(production_collection.head(20))
    with col2:
        st.subheader("Coleção × grupo")
        collection_group = pd.pivot_table(df, index="Colecao", columns="Grupo", values=PRODUCTION_COL, aggfunc="sum", fill_value=0)
        st.dataframe(collection_group, use_container_width=True)

    st.subheader("Ranking e Pareto de referências")
    reference_ranking = (df.groupby(["Referência", "Descrição"], dropna=False)
                         ...
                         .reset_index())
    reference_ranking["% acumulado"] = reference_ranking["Peças aprovadas"].cumsum().div(total_production if total_production else 1).mul(100)
